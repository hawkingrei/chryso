...
fn snapshot_plan_explain() {
    let input = load_json::<PlanTestSuite>("tests/testdata/plan/case1/in.json");
    let expected = load_json::<PlanTestOutputSuite>("tests/testdata/plan/case1/out.json");
    ...
    if should_record() {
        write_json("tests/testdata/plan/case1/out.json", &actual);
        return;
    }
